...
class SkDropInnerShadowImageFilter final : public SkImageFilter_Base {
public:
    SkDropInnerShadowImageFilter(SkScalar dx, SkScalar dy, SkScalar sigmaX, SkScalar sigmaY,
    ...
    static sk_sp<SkImageFilter> Make(SkScalar dx, SkScalar dy, SkScalar sigmaX, SkScalar sigmaY,
                                     SkColor color, bool shadowOnly, sk_sp<SkImageFilter> input,
                                     const SkRect* cropRect) {
        return sk_sp<SkImageFilter>(new SkDropInnerShadowImageFilter(
...
private:
    friend void ::SkRegisterDropInnerShadowImageFilterFlattenable();
    SK_FLATTENABLE_HOOKS(SkDropInnerShadowImageFilter)
...
sk_sp<SkImageFilter> SkMyImageFilters::DropInnerShadow(
        SkScalar dx, SkScalar dy, SkScalar sigmaX, SkScalar sigmaY, SkColor color,
        sk_sp<SkImageFilter> input, const CropRect& cropRect) {
    return SkDropInnerShadowImageFilter::Make(dx, dy, sigmaX, sigmaY, color, /* shadowOnly */ false,
                                         std::move(input), cropRect);
}

sk_sp<SkImageFilter> SkMyImageFilters::DropInnerShadowOnly(
        SkScalar dx, SkScalar dy, SkScalar sigmaX, SkScalar sigmaY, SkColor color,
        sk_sp<SkImageFilter> input, const CropRect& cropRect) {
    return SkDropInnerShadowImageFilter::Make(dx, dy, sigmaX, sigmaY, color, /* shadowOnly */ true,
                                         std::move(input), cropRect);
}

void SkRegisterDropInnerShadowImageFilterFlattenable() {
    SK_REGISTER_FLATTENABLE(SkDropInnerShadowImageFilter);
    // TODO (michaelludwig) - Remove after grace period for SKPs to stop using old name
    SkFlattenable::Register("SkDropInnerShadowImageFilterImpl", SkDropInnerShadowImageFilter::CreateProc);
}

sk_sp<SkFlattenable> SkDropInnerShadowImageFilter::CreateProc(SkReadBuffer& buffer) {
    ...
    // is equivalent to the bool that SkDropInnerShadowImageFilter now uses.
    bool shadowOnly = SkToBool(buffer.read32LE(1));
    return SkDropInnerShadowImageFilter::Make(dx, dy, sigmaX, sigmaY, color, shadowOnly,
                                         common.getInput(0), common.cropRect());
}

void SkDropInnerShadowImageFilter::flatten(SkWriteBuffer& buffer) const {
...
sk_sp<SkSpecialImage> SkDropInnerShadowImageFilter::onFilterImage(const Context& ctx,
    ...
    paint.setColorFilter(SkColorFilters::Blend(fColor, SkBlendMode::kSrcOut));
...
SkRect SkDropInnerShadowImageFilter::computeFastBounds(const SkRect& src) const {
...
SkIRect SkDropInnerShadowImageFilter::onFilterNodeBounds(
